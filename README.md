...
> 📐 **What counts toward scope:** "File" and "lines" refer to your Solana program's source code only (`.rs` files containing your program logic — e.g. `lib.rs`, `state.rs`, `instructions/*.rs`). Test files, config files (`Anchor.toml`, `Cargo.toml`), and auto-generated files (IDL, `target/`) are not counted. **Both the line count AND the file count must be within a tier's limits (AND, not OR) — this applies to every tier:** e.g. Spot within both 150 lines AND 1–2 files, Emergency within both 300 lines AND 2–3 files, Full within both 500 lines AND 5 files. **If either the line count or the file count is exceeded, the job moves up to the next tier — or needs a custom quote if it exceeds the top tier.**
>
> 🚫 **Custom-quote exceptions (all tiers):** Contracts involving external oracle integrations (Pyth, Switchboard), complex cross-program invocation (CPI) to third-party programs, or custom/novel cryptography require a custom quote **regardless of line or file count**, due to the additional verification work involved. The quote is provided **before work begins — there is no mid-audit renegotiation.**
>
